...
class MathProblemAdapter(
    ...
    interface OnClickMath {
        ...
        fun onCLickViewPost(type:TeacherProblemResponse.Body)
    }
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.itemView.rlImg.setOnClickListener {
            onClickMath.onCLickViewPost(list[position])

        }
        if (typeUser == "1") {
            holder.itemView.ivDots.visibility = View.GONE
            holder.itemView.llProfile.visibility = View.VISIBLE
            Glide.with(ctn).load(Constants.IMAGE_URL + list[position].user.image)
        ...
        holder.itemView.tvTime.text =
            Constants.getNotificationTime(list[position].createdAt.toLong())
        ...
        if (list[position].type == 1) {
            Glide.with(ctn).load(Constants.documents_URL + list[position].document)
                .placeholder(R.drawable.profile_unselected)
                .into(holder.itemView.ivImg)
            holder.itemView.ivVideoPlay.visibility=View.GONE
            holder.itemView.ivImg.scaleType = ImageView.ScaleType.CENTER_CROP
        }
        else if (list[position].type == 3) {
            holder.itemView.ivVideoPlay.visibility=View.GONE
            holder.itemView.ivImg.setImageResource(R.drawable.ic_pdf)
        }
